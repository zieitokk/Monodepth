...
def bilinear_sampler_1d_h(input_images, x_offset, wrap_mode='border', name='bilinear_sampler', **kwargs):
    ...
    :param input_images: input images with dimension (batch size, height, width, channels)
    :param x_offset: disparity for project the input images into its stereo pair
    :param wrap_mode: mode for the edge of images, 'border' for constant padding along height and width dim, 'edge' for
                      none padding
    :param name: only for tensorflow to construct the graph by name
    :param kwargs:
